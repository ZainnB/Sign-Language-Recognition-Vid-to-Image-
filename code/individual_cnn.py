...
def load_images(data_dir, label_encoder=None, fit_encoder=False):
    ...
    for img_name in sorted(os.listdir(data_dir)):
        if img_name.endswith('.png'):
            ...
            if not set(np.unique(img)).issubset({0, 255}):
                raise ValueError(f"Image {path} is not binary!")

            img = img / 255.0  # Normalize
            X.append(img)

            # Extract base sign name (remove _1/_2 suffix)
...
# === Train and Evaluate for One Landmark ===
def train_and_evaluate(landmark_name, label_encoder):
    ...
    X_train, y_train = load_images(train_dir, label_encoder=label_encoder)
...
if __name__ == "__main__":
    # 1. Fit label encoder globally on ALL train data from all landmarks
    print("Fitting LabelEncoder on all classes...")
    label_encoder = LabelEncoder()
    all_labels = []

    for lm_name in LANDMARK_NAMES:
        train_dir = f"PSL_Dictionary/landmark_images/train/{lm_name}"
    ...
    print("Saved global label_encoder.pkl")
    ...
    for lm_name in LANDMARK_NAMES:
        try:
            model, acc = train_and_evaluate(lm_name, label_encoder)
            all_accuracies[lm_name] = acc
            all_models[lm_name] = model
        except Exception as e:
            print(f"⚠️ Error processing {lm_name}: {e}")

    # 3. Save models
    os.makedirs("models", exist_ok=True)
    for lm_name, model in all_models.items():
        model_path = f"models/{lm_name}_model.keras"
        model.save(model_path)
        print(f"Saved model for {lm_name} at {model_path}")
    ...
    # 5. Save all accuracies in a text file
    with open("all_landmark_accuracies.txt", "w") as f:
        for lm_name, acc in sorted_acc:
            f.write(f"{lm_name}: {acc*100:.2f}%\n")
    print("Saved accuracies to all_landmark_accuracies.txt")

    # 6. Save a JSON summary for documentation & model aggregation
    summary_data = {
        "classes": label_encoder.classes_.tolist(),
        "accuracies": {lm: float(acc) for lm, acc in all_accuracies.items()},
        "models": {lm: f"models/{lm}_model.keras" for lm in all_models.keys()},
        "top5": [lm for lm, _ in top5]
    }
